...
import { useCreateEducation } from "@/api/hooks/useEducation";
import { useCreateActivity } from "@/api/hooks/useActivities";
import { useCreateCompetition } from "@/api/hooks/useCompetitions";
import { useCreateCamp } from "@/api/hooks/useCamps";
import { useCreateTrip } from "@/api/hooks/useTrips";
import { useCreateGrowth } from "@/api/hooks/useGrowth";
...
export function QuickAddDialog({ open, onOpenChange, defaultKidId, defaultCategory }: QuickAddDialogProps) {
  ...
  // Mutation hooks — backend auto-creates timeline events
  const createEducation = useCreateEducation(kidId);
  const createActivity = useCreateActivity(kidId);
  const createCompetition = useCreateCompetition(kidId);
  const createCamp = useCreateCamp(kidId);
  const createTrip = useCreateTrip();
  const createGrowth = useCreateGrowth(kidId);
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Create the category-specific entity — backend auto-creates timeline event
      switch (category) {
        case "Education":
          if (!schoolName) {
        ...
        case "Trip":
          ...
          await createTrip.mutateAsync({
            tripType,
            kidId: tripType === "Family" ? undefined : kidId,
            tripName,
            destination,
            startDate: date,
            description: description || undefined,
